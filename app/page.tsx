...
export default function Home() {
  ...
  return (
    <main className="relative isolate text-[#3A3A38]" style={{ fontFamily: "'Times New Roman', Times, serif" }}>
      ...
      <section ref={heroRef} className="relative h-[100vh] w-full overflow-hidden">
        ...
        <div className="hero-text relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
          <h1 className="max-w-3xl font-serif text-5xl font-bold leading-tight text-[#F6D86B] drop-shadow-[0_2px_18px_rgba(246,216,107,0.45)] sm:text-7xl">
            You are on a better path.
          </h1>

          <Link href="/book" className="mt-10 rounded-full bg-[#F6D86B] px-10 py-4 text-base font-bold uppercase tracking-widest text-[#3A3A38] transition-transform hover:-translate-y-1 hover:shadow-[0_8px_20px_rgba(246,216,107,0.3)]">
            Book a Session
          </Link>
        </div>
      </section>
      ...
      <section className="journey-section relative mx-auto flex min-h-[180vh] w-full flex-col items-center overflow-hidden px-6 pt-32 text-center">
        ...
        <div className="relative flex w-full max-w-2xl flex-col items-center pb-0">
          ...
          {/* THE INTRODUCTORY OPAQUE BOX */}
          <div className="journey-node relative z-10 mb-20 flex w-full flex-col items-center rounded-3xl border border-[#FBF8F2]/40 bg-[#FBF8F2]/95 p-8 shadow-2xl sm:p-12">
            <p className="mb-6 text-xl leading-relaxed text-[#3A3A38] sm:text-2xl">
            ...
            <p className="text-xl leading-relaxed text-[#3A3A38] sm:text-2xl">
          ...
          {JOURNEY_NODES.map((node, i) => (
            <div key={node.num} className="flex w-full flex-col items-center">
              <div className="journey-node relative flex flex-col items-center gap-4 py-8">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border-2 border-[#4F6F52] bg-[#FBF8F2]/10 font-serif text-xl font-bold text-[#FBF8F2]">{node.num}</div>
                <h3 className="font-serif text-3xl font-bold text-[#FBF8F2] sm:text-4xl">{node.title}</h3>
                <p className="max-w-md text-lg leading-relaxed text-[#FBF8F2]/90">{node.desc}</p>
                
                {/* Standard <a> tag used for mailto: so Next.js doesn't block the email client */}
                {node.href.startsWith("mailto:") ? (
                  <a href={node.href} className="mt-4 text-lg font-bold uppercase tracking-widest text-[#F6D86B] hover:text-white transition-colors">
                    {node.linkText}
                  </a>
                ) : (
                  <Link href={node.href} className="mt-4 text-lg font-bold uppercase tracking-widest text-[#F6D86B] hover:text-white transition-colors">
                    {node.linkText}
                  </Link>
                )}
      ...
      {/* 3. SERVICES WE OFFER (With the requested background) */}
      <section className="relative w-full overflow-hidden px-6 py-24 sm:py-32" style={{ backgroundImage: "url('https://cdn.pixabay.com/photo/2013/02/20/11/30/autumn-83761_1280.jpg')", backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <div className="relative z-10 mx-auto mb-16 text-center">
          <p className="mb-2 text-base font-bold uppercase tracking-[0.35em] text-[#CFE3E8]">What We Do</p>
          <h2 className="font-serif text-4xl font-bold tracking-wide text-[#FBF8F2] sm:text-6xl">
            Services We Offer
          </h2>
        </div>
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-8">
          <div className="service-card flex flex-col rounded-3xl border border-[#3A3A38]/10 bg-[#FBF8F2]/95 p-10 shadow-[0_8px_30px_rgba(0,0,0,0.03)] transition-all hover:-translate-y-2 hover:shadow-xl">
            ...
            <h3 className="mb-4 font-serif text-3xl font-bold text-[#3A3A38]">For Clients</h3>
            <ul className="flex flex-col gap-3 text-lg leading-relaxed text-[#3A3A38]/90 list-disc pl-5 marker:text-[#A65D47]">
          ...
          <div className="service-card flex flex-col rounded-3xl border border-[#3A3A38]/10 bg-[#FBF8F2]/95 p-10 shadow-[0_8px_30px_rgba(0,0,0,0.03)] transition-all hover:-translate-y-2 hover:shadow-xl">
            ...
            <h3 className="mb-4 font-serif text-3xl font-bold text-[#3A3A38]">For Students</h3>
            <ul className="flex flex-col gap-3 text-lg leading-relaxed text-[#3A3A38]/90 list-disc pl-5 marker:text-[#4F6F52]">
              <li>Courses</li>
              <li>Internships</li>
          ...
          <div className="service-card flex flex-col rounded-3xl border border-[#3A3A38]/10 bg-[#FBF8F2]/95 p-10 shadow-[0_8px_30px_rgba(0,0,0,0.03)] transition-all hover:-translate-y-2 hover:shadow-xl">
            ...
            <h3 className="mb-4 font-serif text-3xl font-bold text-[#3A3A38]">For Professionals</h3>
            <ul className="flex flex-col gap-3 text-lg leading-relaxed text-[#3A3A38]/90 list-disc pl-5 marker:text-[#2C4C5B]">
      ...
      {/* 4. BRIDGE LINKS */}
      <section className="relative w-full bg-[#FBF8F2] px-6 pb-40 pt-10">
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-8">
          <Link href="/events" ref={(el) => { linksRef.current[0] = el; }} className="group flex flex-col items-center justify-center rounded-3xl border border-[#3A3A38]/5 bg-white/60 p-10 text-center shadow-[0_8px_30px_rgba(0,0,0,0.03)] backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:bg-white/90 hover:shadow-xl">
            ...
            <h3 className="mb-3 font-serif text-3xl font-bold text-[#3A3A38]">Events & Initiatives</h3>
            <p className="mb-6 text-base leading-relaxed text-[#3A3A38]/80">Explore our community programs, upcoming meets, and long-term psychological initiatives.</p>
            <span className="mt-auto text-sm font-bold uppercase tracking-widest text-[#4F6F52] group-hover:underline">Discover More</span>
          </Link>
          <Link href="/blogs" ref={(el) => { linksRef.current[1] = el; }} className="group flex flex-col items-center justify-center rounded-3xl border border-[#3A3A38]/5 bg-white/60 p-10 text-center shadow-[0_8px_30px_rgba(0,0,0,0.03)] backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:bg-white/90 hover:shadow-xl">
            ...
            <h3 className="mb-3 font-serif text-3xl font-bold text-[#3A3A38]">Gain Perspective</h3>
            <p className="mb-6 text-base leading-relaxed text-[#3A3A38]/80">Insights, reflections, and psychology-backed guidance written directly by our team.</p>
            <span className="mt-auto text-sm font-bold uppercase tracking-widest text-[#4A6B7C] group-hover:underline">Read Articles</span>
          </Link>
          <Link href="/programs" ref={(el) => { linksRef.current[2] = el; }} className="group flex flex-col items-center justify-center rounded-3xl border border-[#3A3A38]/5 bg-white/60 p-10 text-center shadow-[0_8px_30px_rgba(0,0,0,0.03)] backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:bg-white/90 hover:shadow-xl">
            ...
            <h3 className="mb-3 font-serif text-3xl font-bold text-[#3A3A38]">Grow With Us</h3>
            <p className="mb-6 text-base leading-relaxed text-[#3A3A38]/80">Expand your clinical understanding through our structured courses and community internships.</p>
            <span className="mt-auto text-sm font-bold uppercase tracking-widest text-[#8E7A65] group-hover:underline">Explore Programs</span>
          </Link>
        </div>
      </section>
